...
if __package__ is None or __package__ == "":  # pragma: no cover
    ...
    from vlm.compliance_vlm import (  # type: ignore
        ComplianceVLMInput,
        StandaloneComplianceVLM,
    )
else:
    from .compliance_vlm import ComplianceVLMInput, StandaloneComplianceVLM
...
def _reshape_site_matrix(
    ...
    if out.ndim == 2 and out.shape == (num_sites, width):
        return out
    raise ValueError(f"Invalid shape for site matrix, got {out.shape}, expected ({num_sites},{width})")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--site-names", type=str, default="", help="Comma-separated site names")
    parser.add_argument("--dt", type=float, default=0.02, help="Control dt")
    parser.add_argument("--mode", type=str, default="waiting", choices=["waiting", "wiping", "drawing"])
    parser.add_argument("--object", type=str, default="black ink. vase", help="Target object label")

    parser.add_argument("--replay-npz", type=str, default="", help="Replay npz path")
    parser.add_argument("--left-key", type=str, default="", help="left image key in npz")
    parser.add_argument("--right-key", type=str, default="", help="right image key in npz")
    parser.add_argument("--x-obs-key", type=str, default="", help="x_obs key in npz")
    parser.add_argument("--x-wrench-key", type=str, default="", help="x_wrench key in npz")
    parser.add_argument("--head-pos-key", type=str, default="", help="head position key in npz")
    parser.add_argument("--head-quat-key", type=str, default="", help="head quaternion key in npz")

    parser.add_argument("--steps", type=int, default=300, help="Dummy steps if no replay")
    parser.add_argument("--save", type=str, default="", help="Output npz for pose/command")
...
def main() -> None:
    ...
    try:
        if len(args.replay_npz) > 0:
            ...
            left_key = args.left_key if len(args.left_key) > 0 else _find_key(replay, ["left_image", "image", "images", "rgb", "camera"])
            right_key = args.right_key if len(args.right_key) > 0 else _find_key(replay, ["right_image", "image_right", "right"])
            x_obs_key = args.x_obs_key if len(args.x_obs_key) > 0 else _find_key(replay, ["x_obs", "pose", "ee_pose"])
            x_wrench_key = args.x_wrench_key if len(args.x_wrench_key) > 0 else _find_key(replay, ["x_wrench", "wrench", "wrenches"])
            head_pos_key = args.head_pos_key if len(args.head_pos_key) > 0 else _find_key(replay, ["head_pos", "head_position", "head_position_world"])
            head_quat_key = args.head_quat_key if len(args.head_quat_key) > 0 else _find_key(replay, ["head_quat", "head_quaternion", "head_quaternion_world_wxyz"])
            ...
            for i in range(T):
                ...
                right_image = _to_hwc_u8(right_images[i]) if right_images is not None else left_image

                x_obs = _reshape_site_matrix(np.asarray(x_obs_arr[i]), policy.num_sites, 6)

                x_wrench = None
                if x_wrench_arr is not None:
                    x_wrench = _reshape_site_matrix(np.asarray(x_wrench_arr[i]), policy.num_sites, 6)
                ...
                head_quat = None
                if head_quat_arr is not None:
                    head_quat = np.asarray(head_quat_arr[i], dtype=np.float32).reshape(4)
