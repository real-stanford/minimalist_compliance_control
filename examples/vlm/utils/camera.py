...
class Camera:
    ...
    def __init__(
        ...
        if device is None:
            env_name = "VLM_LEFT_CAMERA_ID" if self.side == "left" else "VLM_RIGHT_CAMERA_ID"
        ...
        if not self.cap.isOpened():
            raise RuntimeError(f"Failed to open camera device {self.device} ({self.side}).")
    ...
    def get_frame(self) -> np.ndarray:
        ...
        if not ok or frame is None:
            raise RuntimeError(f"Failed to capture frame from camera {self.device} ({self.side}).")
        return frame
