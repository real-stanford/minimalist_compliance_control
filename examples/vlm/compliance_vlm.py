...
class StandaloneComplianceVLM:
    ...
    def __init__(
        ...
        self.wrench_site_names = list(site_names) if site_names is not None else list(default_site)
        ...
        default_draw = Path("results/toddlerbot_2xm_compliance_vlm_real_world_[time_str]/affordance_draw")
        default_wipe = Path("results/toddlerbot_2xm_compliance_vlm_real_world_[time_str]/affordance_wipe")
        self.fixed_draw_trajectory_path = (
            Path(fixed_draw_trajectory_path) if fixed_draw_trajectory_path else (default_draw / "trajectory.lz4")
        )
        self.fixed_wipe_trajectory_path = (
            Path(fixed_wipe_trajectory_path) if fixed_wipe_trajectory_path else (default_wipe / "trajectory.lz4")
        )
        ...
        self.default_head_pos_world = np.asarray(default_head_pos_world, dtype=np.float32).reshape(3)
        self.default_head_quat_world_wxyz = np.asarray(default_head_quat_world_wxyz, dtype=np.float32).reshape(4)
    ...
    def set_mode(
        ...
        self.args_payload["site"] = self.target_site_names if self.target_site_names else ""
    ...
    def _apply_mode_message(self, msg: object) -> None:
        ...
        if cmd in ("w", "wipe", "wiping"):
            ...
            self.set_mode(False, label_val if label_val else None, sites_val if sites_val else None)
    ...
    def ensure_video_writer(self, frame: np.ndarray) -> bool:
        ...
        if self.video_temp_dir is None:
            self.video_temp_dir = tempfile.TemporaryDirectory(prefix="compliance_vlm_video_")
        ...
        if not writer.isOpened():
            writer.release()
            print("[ComplianceVLM] Warning: failed to open video writer; disabling video logging.")
            self.video_logging_active = False
    ...
    def start_video_capture_thread(self) -> None:
        if self.video_capture_thread is not None and self.video_capture_thread.is_alive():
            return
    ...
    def log_camera_frame(self, timestamp: float, left_only: Optional[bool] = None) -> None:
        del timestamp
    ...
    def export_camera_video(self, output_dir: Path) -> None:
        ...
        if timestamps.size == 0 or self.video_path is None or not self.video_path.exists():
            return
        ...
        if not cap.isOpened():
            ...
            print(f"[ComplianceVLM] Warning: failed to reopen video; copied {dest_path}.")
            return
        ...
        if not writer.isOpened():
            ...
            cap.release()
            shutil.copy2(self.video_path, dest_path)
            print("[ComplianceVLM] Warning: failed to open re-encode writer; copied video.")
            return
        ...
        writer.release()
        cap.release()
        print(f"[ComplianceVLM] Saved camera video at {actual_fps:.2f} FPS to {dest_path}")
    ...
    def maybe_start_prediction(self, inp: ComplianceVLMInput) -> None:
        ...
        head_pos, head_quat = self._get_head_pose(inp.head_pos_world, inp.head_quat_world_wxyz)
    ...
    def _build_command_matrix(
        ...
        command_matrix = np.zeros((self.num_sites, COMMAND_LAYOUT.width), dtype=np.float32)
        ...
        if x_wrench is not None:
            ...
            if wrench_arr.ndim == 2 and wrench_arr.shape[1] >= 6:
                ...
                command_matrix[:rows, COMMAND_LAYOUT.measured_force] = wrench_arr[:rows, :3]
                command_matrix[:rows, COMMAND_LAYOUT.measured_torque] = wrench_arr[:rows, 3:6]
    ...
    def step(self, inp: ComplianceVLMInput) -> ComplianceVLMOutput:
        ...
        if self.status != "wiping":
            self.wipe_pause_end_time = None
        elif self.wipe_pause_end_time is not None and inp.time >= self.wipe_pause_end_time:
            self.wipe_pause_end_time = None
            self.trajectory_plans = {}
            self.traj_start_time = None
        ...
        if not self.trajectory_plans and self.prediction_future is None and not in_pause:
        ...
        if self.prediction_future is not None and self.prediction_future.done():
            ...
            if result is None:
                if wiping_done:
                    print("[ComplianceVLM] Wiping complete; stopping affordance prediction requests.")
                    self.prediction_requested = False
                    self.status = "waiting"
            ...
            if traj_done and self.status == "wiping" and not self.prediction_future:
                if self.fixed_trajectory_active:
                    ...
                    if self.wipe_pause_end_time is None:
                        self.wipe_pause_end_time = float(inp.time) + self.wipe_pause_duration
                else:
                    self.request_prediction_after_completion()
                ...
                if src.exists():
                    ...
                    for idx in range(self.prediction_counter):
                        ...
                        try:
                            ...
                            print(f"[ComplianceVLM] Warning: failed to write args.json: {exc}")
