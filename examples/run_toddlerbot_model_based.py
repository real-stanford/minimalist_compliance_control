...
components from toddlerbot_internal under `model_based/hybrid_servo`.
...
from model_based.hybrid_servo.demo.two_hand_rotate_ball.ochs_helpers import (
    compute_center_quaternion_from_hands,
    compute_ochs_inputs,
    generate_constraint_jacobian,
)
from model_based.zmq_control import KeyboardControlReceiver
...
def main() -> None:
    ...
    base_config_path = os.path.join(repo_root, "config", "toddlerbot.gin")
    model_based_config_path = os.path.join(
        repo_root, "config", "toddlerbot_model_based.gin"
    )
    gin.clear_config()
    gin.parse_config_file(base_config_path, skip_unknown=True)
    gin.parse_config_file(model_based_config_path, skip_unknown=True)
        ...
        while True:
            ...
            if runtime.phase == "prep":
                ...
                if mode_changed:
                    ...
                    _reset_approach_interp(runtime)
                    runtime.wrench_command[:] = 0.0
                    print("[model_based] Phase transition: mode switch -> approach")
                else:
