...
class StandaloneComplianceDP:
    ...
    def __init__(
        ...
        self.pos_damping = np.tile(
            np.diag(2.0 * np.sqrt(np.array([pos_kp, pos_kp, pos_kp], dtype=np.float32))).reshape(1, 9),
            (self.num_sites, 1),
        )
        self.rot_damping = np.tile(
            np.diag(2.0 * np.sqrt(np.array([rot_kp, rot_kp, rot_kp], dtype=np.float32))).reshape(1, 9),
            (self.num_sites, 1),
        )
    ...
    def _read_config_from_process(self, timeout_s: float = 30.0) -> DPConfig:
        ...
        while True:
            ...
            if remaining <= 0:
                raise RuntimeError("Timed out waiting for DP config from inference process")
            try:
            ...
            for source in self.obs_source:
                if source == "x_obs":
                    if inp.x_obs is None:
                        raise ValueError("obs_source contains x_obs but x_obs is missing")
                    components.append(np.asarray(inp.x_obs, dtype=np.float32).reshape(-1))
                elif source == "x_wrench":
                    if inp.x_wrench is None:
                        raise ValueError("obs_source contains x_wrench but x_wrench is missing")
                    components.append(np.asarray(inp.x_wrench, dtype=np.float32).reshape(-1))
                elif source == "obs_motor_pos":
                    if inp.motor_pos is None:
                        raise ValueError("obs_source contains obs_motor_pos but motor_pos is missing")
                    components.append(np.asarray(inp.motor_pos, dtype=np.float32).reshape(-1))
                else:
                    raise ValueError(f"Unsupported obs_source token: {source}")

            obs_vec = np.concatenate(components, axis=0) if len(components) > 1 else components[0]
        ...
        if not self.model_action_seq:
            ...
            for timestamp, action in new_seq:
                while prev_idx < prev_len and prev_seq[prev_idx][0] < timestamp - tol:
                    prev_idx += 1
                if prev_idx < prev_len and abs(prev_seq[prev_idx][0] - timestamp) <= tol:
    ...
    def step(self, inp: ComplianceDPInput) -> ComplianceDPOutput:
        ...
        if self.model_action_seq and now - self.action_seq_timestamp > self.action_max_age_s:
            self.model_action_seq = []
