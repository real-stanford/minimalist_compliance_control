...
class DPModel:
    ...
    def load_model(self, ckpt_path: str, stats: Optional[dict] = None, ckpt: Optional[dict] = None) -> None:
        assert torch is not None
    ...
    def prepare_inputs(
        ...
        if obs_values.shape[0] > self.obs_horizon:
            ...
            pad = np.repeat(obs_values[-1:], self.obs_horizon - obs_values.shape[0], axis=0)
    ...
    def inference_ddim(self, obs_cond: Any, nsteps: int = 10, naction: Optional[Any] = None) -> Any:
        assert torch is not None
        if naction is None:
    ...
    def inference_ddpm(self, obs_cond: Any, nsteps: int, naction: Optional[Any] = None) -> Any:
        assert torch is not None
        if naction is None:
            naction = torch.randn((1, self.pred_horizon, self.action_dim), device=self.device)
