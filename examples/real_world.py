"""Real hardware backend used by examples/run_policy.py.
...
class RealWorld(BaseSim):
    ...
    def __init__(
        ...
        if "arx" in self.robot_name.lower():
            ...
            self.controller = _import_dynamixel_cpp()

        if "arx" in self.robot_name.lower():
            port_pattern = os.getenv("ARX5_INTERFACE", "can[0-9]+")
            baudrate = 0
        elif "leap" in self.robot_name.lower():
            port_pattern = (
                r"cu\.usbserial-.*"
    ...
    def get_observation(self, retries: int = 0) -> Obs:
        ...
        if (
            quat is not None
            and self.is_toddlerbot
            and hasattr(self, "use_balance")
            and bool(self.use_balance)
        ):
            rot_obs = R.from_quat(np.asarray(quat, dtype=np.float32), scalar_first=True)
            euler = rot_obs.as_euler("xyz")
